...
function calcTrabajo(t, data) {
  ...
  const acumular = (item, tipoLabel, montoOverride) => {
    ...
    if (!desgloseMap[key]) desgloseMap[key] = { nombre: pagadorNombre(data, item.pagadoPor, item.cuentaId), tipoLabel, monto: 0 };
...
function pagadorNombre(data, pagadoPor, cuentaId) {
  if (!pagadoPor || pagadoPor === "empresa") {
    const cuenta = cuentaId ? data.cuentas.find((c) => c.id === cuentaId) : null;
    return cuenta?.nombre || data.empresaNombre || "Empresa";
  }
    ...
    <div>
      ...
      <div className="space-y-2">
        ...
        {(() => {
          ...
          const totalGastosReporte = c.materiales + c.manoDeObra;
          const gananciaReporte = c.tienePagoReal ? c.gananciaReal : c.ganancia;
          ...
          return (
            <div key={t.id} className="card">
              ...
              {open && (
                <div style={{ borderTop: `1px dashed ${LINE}` }} className="p-4 pt-3 space-y-3">
                  ...
                  <div className="flex items-center gap-2 border" style={{ borderColor: LINE }}>
                    ...
                    <input className="flex-1 py-1.5 pr-2 text-xs outline-none" value={t.nombre || ""} onChange={(e) => update((d) => { d.trabajos.find((x) => x.id === t.id).nombre = e.target.value; })} />
                  </div>
                  <div>
                    <label className="text-[11px] text-[#7A7263] block mb-0.5">Estimado ($)</label>
                    <input className="ledger-input text-xs" type="number" value={t.estimado ?? ""} onChange={(e) => update((d) => { d.trabajos.find((x) => x.id === t.id).estimado = e.target.value; })} />
                  </div>

                  <div className="card overflow-hidden" style={{ background: "#F5F4F0", borderColor: "#E5E3DC" }}>
                    <div style={{ background: "#000", color: "#fff", padding: "14px 16px" }}>
                      <p style={{ fontSize: "9px", letterSpacing: "1px", color: "#999", margin: "0 0 2px", textTransform: "uppercase" }}>Desglose de gastos</p>
                      <p style={{ fontSize: "15px", fontWeight: 700, margin: 0 }}>{t.apodo || t.nombre} {money(Number(t.estimado || 0))}</p>
                    </div>
                    <div style={{ background: "#fff", padding: "16px" }}>
                      <div className="overflow-x-auto">
                        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "10.5px", minWidth: "520px" }}>
                          <thead><tr style={{ borderBottom: "1px solid #333", color: "#888", fontSize: "9px" }}><td style={{ padding: "4px 0" }}>Fecha</td><td style={{ padding: "4px 0" }}>Descripción</td><td style={{ padding: "4px 0" }}>Categoría</td><td style={{ padding: "4px 0" }}>Pagó</td><td style={{ padding: "4px 0", textAlign: "right" }}>Monto</td></tr></thead>
                          <tbody>
                            {materialesT.map((m) => (
                              <tr key={`mat-${m.id}`} style={{ borderBottom: "1px solid #eee" }}>
                                <td style={{ padding: "6px 0" }}>{fmtDate(m.fecha)}</td>
                                <td style={{ padding: "6px 0" }}>{m.descripcion || "Material"}{m.numeroInvoice ? ` · Invoice #${m.numeroInvoice}` : ""}</td>
                                <td style={{ padding: "6px 0" }}>Materiales</td>
                                <td style={{ padding: "6px 0", color: "#666" }}>{pagadorNombre(data, m.pagadoPor, m.cuentaId)}</td>
                                <td style={{ padding: "6px 0", textAlign: "right", color: RED }}>-{money(materialNeto(m))}</td>
                              </tr>
                            ))}
                            {nominaT.map((n) => (
                              <tr key={`nom-${n.id}`} style={{ borderBottom: "1px solid #eee" }}>
                                <td style={{ padding: "6px 0" }}>{fmtDate(n.fecha)}</td>
                                <td style={{ padding: "6px 0" }}>{data.empleados.find((e) => e.id === n.empleadoId)?.nombre || "Nómina"}</td>
                                <td style={{ padding: "6px 0" }}>Mano de obra</td>
                                <td style={{ padding: "6px 0", color: "#666" }}>{pagadorNombre(data, n.pagadoPor, n.cuentaId)}</td>
                                <td style={{ padding: "6px 0", textAlign: "right", color: RED }}>-{money(Number(n.monto || 0))}</td>
                              </tr>
                            ))}
                            {materialesT.length === 0 && nominaT.length === 0 && <tr><td colSpan="5" style={{ padding: "10px 0", color: "#999" }}>No hay gastos registrados para este trabajo.</td></tr>}
                          </tbody>
                        </table>
                      </div>
                      <div style={{ display: "flex", justifyContent: "space-between", fontSize: "12px", paddingTop: "8px" }}><span>Acordado</span><span style={{ fontWeight: 600 }}>{money(Number(t.estimado || 0))}</span></div>
                      {c.tienePagoReal && <div style={{ display: "flex", justifyContent: "space-between", fontSize: "12px", paddingTop: "4px" }}><span>Pagó el cliente</span><span style={{ fontWeight: 600 }}>{money(Number(t.estimadoPagado || 0))}</span></div>}
                      <div style={{ display: "flex", justifyContent: "space-between", fontSize: "12px", color: RED, paddingBottom: "6px", paddingTop: "4px" }}><span>Total Gastos</span><span style={{ fontWeight: 600 }}>-{money(totalGastosReporte)}</span></div>
                      <div style={{ display: "flex", justifyContent: "space-between", borderTop: "2px solid #000", paddingTop: "6px" }}><span style={{ fontSize: "14px", fontWeight: 800 }}>GANANCIA</span><span style={{ fontSize: "15px", fontWeight: 800, color: gananciaReporte >= 0 ? GREEN : RED }}>{money(gananciaReporte)}</span></div>
                      <p style={{ fontSize: "9.5px", color: "#999", textTransform: "uppercase", margin: "14px 0 6px" }}>Reparto de ganancia (50/50)</p>
                      <div className="overflow-x-auto">
                        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "11px", minWidth: "430px" }}>
                          <thead><tr style={{ borderBottom: "1px solid #333", color: "#888", fontSize: "9px" }}><td style={{ padding: "3px 0" }}>Entidad</td><td style={{ padding: "3px 0", textAlign: "right" }}>Ganancia</td><td style={{ padding: "3px 0", textAlign: "right" }}>Reembolso</td><td style={{ padding: "3px 0", textAlign: "right" }}>Monto a pagar</td></tr></thead>
                          <tbody>{ENTIDADES_REPARTO.map((entidad, entidadIdx) => {
                            const socio = data.socios[entidadIdx];
                            const reembolso = socio ? c.reembolsoDeSocio(socio.id) : (entidadIdx === 0 ? c.totalReembolsosTrabajo : 0);
                            const gananciaEntidad = c.mitadResto;
                            return <tr key={entidad.id} style={{ borderBottom: entidadIdx === 0 ? "1px solid #eee" : "none" }}><td style={{ padding: "6px 0" }}>{entidad.nombre}</td><td style={{ padding: "6px 0", textAlign: "right" }}>{money(gananciaEntidad)}</td><td style={{ padding: "6px 0", textAlign: "right" }}>{money(reembolso)}</td><td style={{ padding: "6px 0", textAlign: "right", fontWeight: 700, color: GREEN }}>{money(gananciaEntidad + reembolso)}</td></tr>;
                          })}</tbody>
                        </table>
                      </div>
                    </div>
                  ...
                  <div className="mt-2 mb-1">
                    ...
                    <input className="ledger-input text-xs" type="number" placeholder="Déjalo vacío si pagaron el estimado completo" value={t.estimadoPagado ?? ""} onChange={(e) => update((d) => { d.trabajos.find((x) => x.id === t.id).estimadoPagado = e.target.value; })} />
                  </div>
                  {c.tienePagoReal && <Row label="Ganancia real (según lo pagado)" value={money(c.gananciaReal)} bold accent={c.gananciaReal >= 0 ? GREEN : RED} />}
            ...
            <div className="card p-4">
                  ...
                  {pagosDeEstaActividad.map((pago) => {
                    ...
                    if (pagoEditandoId === pago.id) {
                      return (
                        <div key={pago.id} className="border p-2 space-y-2" style={{ borderColor: AMBER, background: "#FBF8F2" }}>
                          ...
                          <select
                            className="ledger-input text-xs"
                            value={pagoEditForm.pagadoPor === "empresa" ? "empresa:" + (pagoEditForm.cuentaId || "") : (pagoEditForm.pagadoPor || "empresa:" + (data.cuentas.find((c) => c.esCuentaAntesSociedad)?.id || ""))}
                            onChange={(e) => {
                              const v = e.target.value;
                              if (v.startsWith("empresa:")) setPagoEditForm({ ...pagoEditForm, pagadoPor: "empresa", cuentaId: v.slice(8) });
                              else setPagoEditForm({ ...pagoEditForm, pagadoPor: v });
                            }}
                          >
                            {data.cuentas.filter((c) => !c.esPersonal).map((c) => (
                              <option key={c.id} value={"empresa:" + c.id}>Pagado desde cuenta {c.nombre}</option>
                            ))}
                    ...
                    return (
                      <div key={pago.id} className="flex justify-between items-center text-[11px] text-[#7A7263]">
                        <span>
                          Pago: <b>{money(pago.monto)}</b> a {empleado?.nombre || "—"} · pagado por {pagadorNombre(data, pago.pagadoPor, pago.cuentaId)}
              ...
              {pagoAbiertoId === b.id ? (
                <div className="border p-2 mb-2 space-y-2" style={{ borderColor: AMBER, background: "#FBF8F2" }}>
                  ...
                  <select
                    className="ledger-input text-xs"
                    value={pagoForm.pagadoPor === "empresa" ? "empresa:" + (pagoForm.cuentaId || "") : (pagoForm.pagadoPor || "empresa:" + (data.cuentas.find((c) => c.esCuentaAntesSociedad)?.id || ""))}
                    onChange={(e) => {
                      const v = e.target.value;
                      if (v.startsWith("empresa:")) setPagoForm({ ...pagoForm, pagadoPor: "empresa", cuentaId: v.slice(8) });
                      else setPagoForm({ ...pagoForm, pagadoPor: v });
                    }}
                  >
                    {data.cuentas.filter((c) => !c.esPersonal).map((c) => (
                      <option key={c.id} value={"empresa:" + c.id}>Pagado desde cuenta {c.nombre}</option>
                  ...
                  <select className="ledger-input text-xs" value={pagoForm.cuentaId || ""} onChange={(e) => setPagoForm({ ...pagoForm, cuentaId: e.target.value })}>
                    <option value="">Cuenta bancaria… (si fue una cuenta personal distinta)</option>
                    {data.cuentas.map((c) => <option key={c.id} value={c.id}>{c.nombre}</option>)}
                  </select>
        ...
        <div className="card p-4">
          ...
          {!payForm ? (
            ...
            <div className="space-y-2">
              ...
              <select
                className="ledger-input"
                value={payForm.pagadoPor === "empresa" ? "empresa:" + (payForm.cuentaId || "") : (payForm.pagadoPor || "empresa:" + (data.cuentas.find((c) => c.esCuentaAntesSociedad)?.id || ""))}
                onChange={(e) => {
                  const v = e.target.value;
                  if (v.startsWith("empresa:")) setPayForm({ ...payForm, pagadoPor: "empresa", cuentaId: v.slice(8) });
                  else setPayForm({ ...payForm, pagadoPor: v });
                }}
              >
                {data.cuentas.filter((c) => !c.esPersonal).map((c) => (
                  <option key={c.id} value={"empresa:" + c.id}>Pagado desde cuenta {c.nombre}</option>
            ...
            <div key={n.id} className="py-1.5 border-b last:border-0" style={{ borderColor: LINE }}>
              {editando ? (
                <div className="space-y-2 py-1">
                  ...
                  <select
                    className="ledger-input text-xs"
                    value={editForm.pagadoPor === "empresa" ? "empresa:" + (editForm.cuentaId || "") : (editForm.pagadoPor || "empresa:" + (data.cuentas.find((c) => c.esCuentaAntesSociedad)?.id || ""))}
                    onChange={(e) => {
                      const v = e.target.value;
                      if (v.startsWith("empresa:")) setEditForm({ ...editForm, pagadoPor: "empresa", cuentaId: v.slice(8) });
                      else setEditForm({ ...editForm, pagadoPor: v });
                    }}
                  >
                    {data.cuentas.filter((c) => !c.esPersonal).map((c) => (
                      <option key={c.id} value={"empresa:" + c.id}>Pagado desde cuenta {c.nombre}</option>
                ...
                <div className="flex justify-between items-center text-sm">
                  <div>
                    ...
                    <div className="text-[11px] text-[#7A7263]">
                      {fmtDate(n.fecha)} · pagado por {pagadorNombre(data, n.pagadoPor, n.cuentaId)}{n.numeroCheque ? ` · cheque #${n.numeroCheque}` : n.formaPago && n.formaPago !== "efectivo" ? ` · ${n.formaPago === "zelle" ? "Zelle" : n.formaPago}` : ""}{n.reembolsado ? " · reembolsado" : ""}
                      {n.antesSociedad && (
                        <span className="ml-1 text-[9px] uppercase px-1 py-0.5" style={{ background: "#FBE9D9", color: AMBER }}>Antes de la sociedad</span>
                      )}
                    </div>
    ...
    <div>
      ...
      {scan?.status === "review" && (
        <div className="card p-4 mb-4">
          ...
          <div className="space-y-2">
            ...
            <select
              className="ledger-input"
              value={scan.pagadoPor === "empresa" ? "empresa:" + (scan.cuentaId || "") : (scan.pagadoPor || "empresa:" + (data.cuentas.find((c) => c.esCuentaAntesSociedad)?.id || ""))}
              onChange={(e) => {
                const v = e.target.value;
                if (v.startsWith("empresa:")) setScan({ ...scan, pagadoPor: "empresa", cuentaId: v.slice(8) });
                else setScan({ ...scan, pagadoPor: v });
              }}
            >
              {data.cuentas.filter((c) => !c.esPersonal).map((c) => (
                <option key={c.id} value={"empresa:" + c.id}>Pagado desde cuenta {c.nombre}</option>
      ...
      {form && (
        <div className="card p-4 mb-4 space-y-2">
          ...
          <select
            className="ledger-input"
            value={form.pagadoPor === "empresa" ? "empresa:" + (form.cuentaId || "") : (form.pagadoPor || "empresa:" + (data.cuentas.find((c) => c.esCuentaAntesSociedad)?.id || ""))}
            onChange={(e) => {
              const v = e.target.value;
              if (v.startsWith("empresa:")) setForm({ ...form, pagadoPor: "empresa", cuentaId: v.slice(8) });
              else setForm({ ...form, pagadoPor: v });
            }}
          >
            {data.cuentas.filter((c) => !c.esPersonal).map((c) => (
              <option key={c.id} value={"empresa:" + c.id}>Pagado desde cuenta {c.nombre}</option>
              ...
              <div key={m.id} className="flex justify-between items-center py-1.5 text-sm border-b last:border-0" style={{ borderColor: LINE }}>
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <div className="text-[11px] text-[#7A7263]">
                      {fmtDate(m.fecha)} · pagado por {pagadorNombre(data, m.pagadoPor, m.cuentaId)}
                    ...
                    {editandoMaterialId === m.id ? (
                      <div className="border p-2 mt-1 space-y-1.5" style={{ borderColor: AMBER, background: "#FBF8F2" }}>
                        ...
                        <select
                          className="ledger-input text-xs"
                          value={editMaterialForm.pagadoPor?.startsWith("empleado:") ? "empleado" : (editMaterialForm.pagadoPor === "empresa" ? "empresa:" + (editMaterialForm.cuentaId || "") : (editMaterialForm.pagadoPor || "empresa:" + (data.cuentas.find((c) => c.esCuentaAntesSociedad)?.id || "")))}
                          onChange={(e) => {
                            const v = e.target.value;
                            if (v.startsWith("empresa:")) setEditMaterialForm({ ...editMaterialForm, pagadoPor: "empresa", cuentaId: v.slice(8) });
                            else setEditMaterialForm({ ...editMaterialForm, pagadoPor: v });
                          }}
                        >
                          {data.cuentas.filter((c) => !c.esPersonal).map((c) => (
                            <option key={c.id} value={"empresa:" + c.id}>Pagado desde cuenta {c.nombre}</option>
            ...
            <div key={grupo.facturaId} className="py-2 border-b last:border-0" style={{ borderColor: LINE }}>
              <div className="flex items-center gap-2 mb-1.5">
                ...
                <div className="text-[12px] text-[#7A7263] flex items-center gap-1 flex-wrap">
                  <span>Factura · {items.length} artículos · {fmtDate(items[0].fecha)} {trab ? `· ${trab.apodo || trab.nombre}` : ""} · pagado por {pagadorNombre(data, items[0].pagadoPor, items[0].cuentaId)}</span>
                  {trab?.estado === "cerrado" && (
                    <span className="text-[9px] uppercase px-1.5 py-0.5" style={{ background: "#E1EEE6", color: GREEN }}>Concluido</span>
                  )}
                </div>
    ...
    <div>
      ...
      {data.cuentas.filter((cuenta) => cuenta.esCuentaAntesSociedad).map((cuenta) => {
        ...
        data.trabajos.forEach((t) => {
          if (t.estado !== "cerrado") return; // solo trabajos ya terminados/cerrados
          const c = calcTrabajo(t, data);
        ...
        return (
          <div key={cuenta.id} className="card p-4 mb-4" style={{ borderColor: GREEN }}>
            <div className="stamp text-[12px] mb-3" style={{ color: GREEN }}>REPOSICIÓN DE CAJA CHICA — {cuenta.nombre.toUpperCase()} (TRABAJOS TERMINADOS)</div>
            <div className="space-y-1.5">
              {filas.map((f) => (
